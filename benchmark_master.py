...
# FRNN runs in an isolated subprocess; baselines run in-process with PyTorch.
import subprocess, sys, json, time, os, tempfile
import numpy as np
import torch
import pynvml
...
def run_frnn_isolated(pts_flat, N, D, R):
    ...
    np.save(tmp.name, pts_flat)
    payload = json.dumps({
        "pts_path": tmp.name, "N": N, "D": D, "K": K,
        "R": R, "warmup": WARMUP, "trials": TRIALS,
    })
    r = subprocess.run(
        [sys.executable, "_run_frnn_isolated.py"],
...
for D in D_SWEEP:
    for N in N_SWEEP:
        ...
        pts_flat = pts_np.flatten()
        key      = f"D{D}_N{N}"
        print(f"\n{'─'*56}\n  {key}  R={R:.5f}\n{'─'*56}")

        try:
            frnn_res = run_frnn_isolated(pts_flat, N, D, R)
        except Exception as e:
